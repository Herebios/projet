#ifndef _ID_ACTIONS_H_
#define _ID_ACTIONS_H_

//joueur
#define JOUEUR_CHANGE_DIR 11
#define JOUEUR_MV 12
#define JOUEUR_MV_TUILE 13
#define ADD_JOUEUR_TUILE 14
#define RM_JOUEUR_TUILE 15

//pour les objets, commence par 2
#define ADD_OBJET_JOUEUR 20
#define RM_OBJET_JOUEUR 21
#define SPAWN_OBJET 22

//pour les casts, commence par 3
// + 0 pour les zones
#define NEW_ZONE 300
#define UPDATE_ZONE 301
#define RM_ZONE 302

//pour les attaques commence par un 4
#define BASIC_ATTACK 400

#endif
