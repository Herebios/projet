...
\end{figure}
    ...
    la fois.
    Après cela, vous arrivez dans une salle d’attente, en attendant que les autres joueurs se joignent à vous. Une fois que la personne jouant le rôle de serveur lance la partie, le jeu démarre, et il n’est plus possible de rejoindre la partie, même si vous étiez déjà dedans, que vous avez quitté le jeu, puis tenté de vous reconnecter. \\
    ...
    La gestion des entités dynamiques et des tuiles de la map est gérée grâce à l’implémentation d’une liste générique. 
    ...
    Cette partie abordera le développement du jeu dans sa globalité. De la création des ressources graphiques à la création du client/serveur en passant par la gestion de la carte, des objets, etc. 
        ...
        Nous allons maintenant présenter les différentes structures de données mises en œuvre pour la réalisation du jeu.
            ...
            Quatre listes tiennent compte des joueurs, des objets, des monstres, et des effets des compétences actives afin de les afficher facilement.
            ...
            Le personnage est représenté par une structure C. Ses statistiques sont stockées deux tableaux de quatre valeurs, 
            ...
            Puis la fonction avancer est appelée et change la position du personnage grâc   e à sa vitesse et sa direction en vérifiant qu’il ne dépasse pas les bords de la tuile. 
            ...
            Elles sont définies, de manière statique dans la structure compétence, et de manière dynamique dans la structure \textit{cast}. Quand une compétence est lancée, la structure de \textit{cast} est créée et ajoutée dans une liste. Le \textit{cast} va évoluer en fonction des appels de la fonction \textit{update}. De sorte à par exemple supprimer une compétence de zone, après sa durée de vie. La fonction d’update permet aussi de détecter des joueurs en collision pour leur infliger des dégâts par exemple. Les \textit{casts} de compétences sont génériques, on peut ainsi avoir plusieurs types de compétences différentes.\\\\
            L’implémentation des \textit{casts} est différente entre le côté client et le côté serveur.\\
            Le client, au moment de la détection d’une touche d’envoi de compétence, fait une requête au serveur avec les paramètres nécessaires, comme les positions etc.\\
            Le serveur ajoute ce nouveau \textit{cast} dans une liste et communique avec les joueurs sur la tuile, les informations comme la position, la taille d’une zone etc.
            ...
            Quarante objets sont présents dans le jeu, séparés en 4 catégories de rareté : 
            \begin{itemize}
                \item commun
                \item rare
                \item épique
            ...
            On effectue d'abord les calculs de somme puis les calculs de produit. Pour ce faire chaque statistiques de l'objet a un couple de valeurs. 
            La première valeur correspond à ce qui est à sommer ou multiplier à la statistiques de base du personnage. 
            ...
            Un vecteur horizontal orienté à droite qui servira de base est créé ainsi qu'un vecteur représentant la direction du personnage vers le mobs. (Voir Figure~\ref{fig:Mob}) 
        ...
        La parallélisation est donc obligatoire.
            ...
            Tout d’abord le client se connecte au serveur, suite au choix réalisé dans le menu et suite à l’initialisation du serveur. 
            ...
            Différents codes peuvent être utilisé lors de l’envoi de paquets (Voir Table~\ref{tab:tabCliServ}): 
            ...
            Pour cela, la fonction \textit{sendf} inspirée de \textit{sprintf} est utilisée pour mettre en forme les informations à envoyer au serveur.\\\\
            ...
            Il renvoie ensuite à tous les joueurs présents sur la tuile, la position de ce joueur après un déplacement ou non.\\\\
        ...
        Cette partie traitera de l'utilisation de la librairie SDL2 qui sert pour le rendu graphique du jeu.
            ...
            \end{figure}
                ...
                \begin{itemize}
                    ...
                    \item un bouton charger paramètres qui charge les paramètres dans ce même fichier
                ...
                Les textes sont quant à eux chargés et sauvegardés dans un tableau de structure \textit{texte\_t} qui contient les mêmes champs que \textit{img\_t}, avec en plus un char* correspondant au texte affiché.\\
                ...
                Les événements en SDL sont gérés à l’aide d’une structure \textit{SDL\_Event} fournie par SDL. Nous nous en servons dans notre projet pour détecter les clics de la souris et les touches pressées au clavier. 
                    ...
                    Les clics souris sont assez simples à gérer : si l’événement est de type clic souris, on crée un \textit{\texttt{SDL\_Point}\footnote{une position sur la fenêtre représentée par deux coordonnées x et y.}} correspondant aux coordonnées du curseur de la souris. De nombreuses conditions testent si le clic de la souris est dans les coordonnées d’un bouton à l’aide de la fonction \textit{SDL\_PointInRect} et de la valeur de \textit{pos\_actuelle}. En fonction des boutons cliqués, différentes actions se passent. Il est possible de naviguer entre les différentes sections du menu de cette manière. 

                    \subparagraph{clavier}\leavevmode\newline
                    Les saisies clavier servent à modifier des chaînes de caractères mais aussi à se déplacer dans les menus à l’aide des flèches et des touches entrées. La saisie de l’adresse IP se fait via un switch dans la fonction \textit{saisie\_touche\_ip} vue précédemment. \\
                    ...
                    \begin{figure}[H]
                        ...
                        \label{fig:codeMenu}
                    \end{figure}
                    Si l’événement SDL est de type \textit{SDL\_KEYDOWN}, on créée un \textit{SDL\_Point} qui correspond aux coordonnées du bouton. \\
