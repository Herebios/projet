...
\end{figure}
	...
	\section{Analyse}
	Nous allons développer différents modules pour structurer le code.
	Il y aura un menu pour le jeu. Différents boutons devront être présents pour lancer le jeu, que ce soit le serveur ou les clients. Le client rejoindra le serveur avec son adresse IP.
	Différents modules vont implémenter les éléments du jeu : les personnages, les objets, les monstres, les compétences.
	Enfin, le code principal du jeu sera divisé entre le client et le serveur. Chacun aura des fonctions pour communiquer en réseau et d'autres de gestion du jeu en trois parties : celles propres au client, celles propres au serveur et celles communes.
	Nous aurons besoin de structures de données. Nous utiliserons une file pour la gestion des messages réseau en interne. La gestion des entités dynamiques des tuiles de la map sera réalisée avec l’implémentation d’une liste générique.
